...
def extract_key_info(body, subject):
    """
    Wyciąga TYLKO kluczowe informacje z emaila, pomija spam i reklamy
    
    Args:
        body (str): Ciało emaila
        subject (str): Temat emaila
    
    Returns:
        dict: Słownik z kluczowymi informacjami
    """
    import re
    ...
    # Numer zamówienia z tematu lub body
    order_match = re.search(r'(?:Zamówienie|Order)\s+(\d{16,})', subject + body)
    if order_match:
        info['order_number'] = order_match.group(1)
    
    # Status z tematu
    if 'potwierdzone' in subject.lower() or 'confirmed' in subject.lower():
        info['status'] = 'zamówienie potwierdzone'
    elif 'wysłane' in subject.lower() or 'shipped' in subject.lower():
        info['status'] = 'zamówienie wysłane'
    elif 'dostarczono' in subject.lower() or 'delivered' in subject.lower():
        info['status'] = 'dostarczono'
    
    # Produkt (pierwsza linia z opisem produktu)
    product_patterns = [
        r'(?:sztuk?|pcs?)\s+(.+?)(?:\n|x\d)',  # "7 sztuk 3 Cal 75mm..."
        r'\n\s*(.+?)\s*x\d+\s*\n',  # Produkt przed "x1"
    ]
    for pattern in product_patterns:
        ...
        if match:
            ...
            # Usuń zbędne znaki
            product = re.sub(r'\.{3,}', '...', product)  # ... na końcu
            if len(product) > 10 and 'spodobać' not in product.lower():
                info['product_name'] = product
                break
    
    # Ilość
    qty_match = re.search(r'x(\d+)', body)
    if qty_match:
        info['quantity'] = f"x{qty_match.group(1)}"
    
    # Cena (kwota zamówienia)
    price_patterns = [
        r'(?:Całkowita kwota|Total|Razem|kwota zamówienia)[:\s]*([0-9,\.]+\s*zł)',
        r'([0-9]+[,\.]\d{2}\s*zł)',
    ]
    ...
    # Adres (po "Wysyłka do" lub "Shipping to")
    address_match = re.search(
        r'(?:Wysyłka do|Shipping to|Adres)[:\s]*\n\s*(.+?)\n\s*(.+?)\n',
        body,
        re.IGNORECASE
    )
    if address_match:
        address_line1 = address_match.group(1).strip()
        address_line2 = address_match.group(2).strip()
        info['address'] = f"{address_line1}, {address_line2}"
    
    # Telefon
    phone_match = re.search(r'\(\+?\d{1,3}\)\s*[\d\s]{9,}', body)
    if phone_match:
        info['phone'] = phone_match.group(0).strip()
    
    # Odbiorca (przed telefonem)
    recipient_match = re.search(r'\n\s*(.+?)\s+\(\+?\d{1,3}\)', body)
    if recipient_match:
        recipient = recipient_match.group(1).strip()
        if len(recipient) < 50:  # Nie za długie
...
def print_key_info(info):
    """Wyświetla kluczowe informacje w czytelny sposób"""
    print("🔍 KLUCZOWE INFORMACJE (bez spamu):")
    print("=" * 80)
    
    if info['order_number']:
        print(f"📦 Numer zamówienia: {info['order_number']}")
    
    if info['status']:
        print(f"📊 Status: {info['status']}")
    
    if info['product_name']:
        print(f"🛍️  Produkt: {info['product_name']}")
    
    if info['quantity']:
        print(f"📏 Ilość: {info['quantity']}")
    
    if info['price']:
        print(f"💰 Cena: {info['price']}")
    
    if info['recipient']:
        print(f"👤 Odbiorca: {info['recipient']}")
